...
export default class DashboardController implements Controller {
  ...
  private getRecommendedStocks = async (req: Request, res: Response): Promise<Response | void> => {
    try {
  ...
  private getFavoriteStocks = async (req: Request, res: Response): Promise<Response | void> => {
    try {
      const userId = req.body.userId;
      let favoriteStocks = await this.dashboardService.findAllFavorite(userId);
      console.log('favorites are', favoriteStocks[0].stocks);
      const favorites = await this.dashboardService.findAllFavoriteStocks(favoriteStocks[0].stocks);
  ...
  private addToFavorites = async (req: Request, res: Response): Promise<Response | void> => {
  ...
  private deleteFromFavorite = async (req: Request, res: Response): Promise<Response | void> => {
    //add a stock id got in req to the favorites table in a current user
    const user = req.body.user;
    const stock = req.body.stock;
