...
export default class BlogsService {
  ...
  public async updateBlogs(blogs: Blogs): Promise<any> {
    try {
      console.log(blogs);
      const comm = await this.blogs.findOneAndUpdate(
        { userID: blogs.userID, blogsID: blogs.blogsID },
        { title: blogs.title, description: blogs.description, content: blogs.content },
      );
      console.log('Success');
      return comm;
    } catch (err: any) {
