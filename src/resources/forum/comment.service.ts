...
export default class CommentService {
  ...
  public async updateComment(comment: Comment): Promise<any> {
    try {
      console.log(comment);
      const comm = await this.comment.findOneAndUpdate(
        { symbol: comment.symbol, commentID: comment.commentID },
        { comment: comment.comment, creation_date: comment.creation_date },
      );
      console.log('Success');
      return comm;
    } catch (err: any) {
